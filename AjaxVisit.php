...
WHERE c.grId = g.groupId  LIMIT $from,$rows;";
	...
	else if($_GET['action'] == "investigationNames")
	{
		if(isset($_GET['page'])&&(isset($_GET['num'])))
		{
			...
			$totalCnt = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM investigationmst"),0);
			// Getting the total number of pages. Always round up using ceil() 
		$total_pages = ceil($totalCnt / $rows);
	...
	else if($_GET['action'] == "medicineNames")
	{
		if(isset($_GET['page'])&&(isset($_GET['num'])))
		{
			...
			$totalCnt = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM medicine"),0);
			// Getting the total number of pages. Always round up using ceil() 
		$total_pages = ceil($totalCnt / $rows);
	...
	else if($_GET['action'] == "frequencyNames")
	{
		if(isset($_GET['page'])&&(isset($_GET['num'])))
		{
			$even = false;
			...
			$totalCnt = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM frequency"),0);
			// Getting the total number of pages. Always round up using ceil() 
		$total_pages = ceil($totalCnt / $rows);
			...
			$query = "SELECT frequencyId, frequency FROM frequency LIMIT $from, 10;";
			$rows = mysql_query($query, $HMS) or die(mysql_error());
			$row = mysql_fetch_assoc($rows);
			$totalRows = mysql_num_rows($rows);
			...
			echo '<table border="1" cellspacing="50" cellpadding="10" bordercolor="#D6D6D6" class="ui-widget" id="users-contain">
					<tr>
					<th class="head" width="3%">Select</th>
					<th class="head" width="20%">Duration</th>					
					</tr>';
			do{
				if($even == true)
				{
				...
				echo '<td><input type="radio" name="medicine" value="'.$row['frequencyId'].'"></td>';
			...
			echo '<script type="text/javascript">
						function nextPage()
						{
							$.ajax({
								url: "AjaxVisit.php",
								data: "action=frequencyNames&page='.($_GET['page']+1).'&num='.$_GET['num'].'",
								success: function(data) {
									$("#dialog-form").html(data);
