...
$total_rows  = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM investigationgr where groupName LIKE '".$name."'"),0);
...
$from = (($page * $rows) - $rows);
mysql_select_db($database_HMS, $HMS);
$query_group = "SELECT * FROM investigationgr where groupName LIKE '".$name."' LIMIT $from,$rows";
$group = mysql_query($query_group, $HMS) or die(mysql_error());
$row_group = mysql_fetch_assoc($group);
$total_rows  = mysql_num_rows($group);
  ...
  <tr>    
   ...
   do {
		...
		else
		{
		 ...
		 echo '<td>'.$row_group['groupName'].'</td>';
		 
      	 echo '<td class="options-width">
		 	
			<a title="Edit" onclick="update_submit('.$row_group['groupId'].')" class="icon-1 info-tooltip"></a>
			<a title="Delete" onclick="delete_confirm('.$row_group['groupId'].')" class="icon-2 info-tooltip"></a>
      </td>
    </tr>';
